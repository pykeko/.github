...
Under the hood it is [**Moorhen**](https://github.com/moorhen-coot/Moorhen): Coot's C++ engine (libcootapi, CCP4, Clipper, MMDB, GEMMI, RDKit) compiled to WebAssembly and wrapped in a TypeScript/React interface. PyKeko packages that into a self-contained, one-click `.app` / `.dmg` and tunes it to feel like the **Coot 0.9.x** crystallographers know — the same keyboard shortcuts, defaults, and workflow primitives — while adding features of its own: a [PyMOL](https://www.pymol.org/) command-language scripting console, command-line integration, a backbone/sidechain torsion editor, Claude/MCP control, and more.
